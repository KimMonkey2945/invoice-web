...
- 수락 기준과 완료 조건 정의
- **모든 구현 작업은 완료 직후 반드시 테스트를 수행하며, 테스트는 예외 없이 Playwright MCP로 진행** (자세한 규칙은 아래 "🧪 테스트 원칙 (필수)" 참조)
- **API 연동 및 비즈니스 로직 구현 시 정상 경로 + 에러/예외 + 엣지 케이스를 모두 포함하는 꼼꼼한 시나리오로 검증**
- **테스트를 수행하지 않았거나 실패한 Task는 절대 완료(✅)로 표시하지 않음** (테스트 통과가 완료의 전제 조건)
...
모든 로드맵과 작업 지침에는 다음 테스트 원칙이 반드시 반영되어야 합니다. 이는 권장이 아니라 **강제 규칙**입니다.

1. **구현 후 반드시 테스트**: 모든 구현 작업은 완료 직후 예외 없이 테스트를 수행합니다. 테스트 없이 다음 단계로 넘어가지 않습니다.
2. **Playwright MCP 사용**: 모든 기능 테스트(E2E·통합·UI 플로우)는 **Playwright MCP**를 사용해 수행합니다. 다른 수단으로 테스트를 대체하지 않습니다.
3. **API 연동·비즈니스 로직은 꼼꼼하게**: 이러한 작업은 **정상 경로 + 에러/예외 처리 + 엣지 케이스**까지 모두 검증합니다. (예: 정상 응답, 데이터 없음/404, 유효성 실패, 권한/만료, 네트워크 오류, 경계값)
4. **작업 파일에 테스트 체크리스트 포함**: 각 구현 Task에는 **"## 테스트 체크리스트"** 섹션과 구체적인 Playwright MCP 테스트 시나리오를 반드시 작성합니다.
5. **테스트 통과 = 완료 기준(Definition of Done)**: 테스트를 수행하지 않았거나 실패한 Task는 완료(✅)로 표시하지 않습니다. 테스트 통과가 확인된 후에만 완료 처리합니다.
...
- 작업 파일의 명세서를 따름
- 기능과 기능성 구현
- **구현이 끝나면 반드시 Playwright MCP로 테스트를 수행** (테스트는 선택이 아닌 필수 단계)
- **API 연동 및 비즈니스 로직은 정상/에러/엣지 케이스를 모두 포함해 꼼꼼히 검증**
- 각 단계 후 작업 파일 내 단계 진행 상황 업데이트
- 구현 완료 후 Playwright MCP를 사용한 E2E 테스트 실행 → **통과를 확인한 후에만** 다음 단계로 진행
...
- **Task 005: 데이터베이스 및 API 개발** - 우선순위
  ...
  - **(구현 직후 필수 테스트)** Playwright MCP로 API 엔드포인트 통합 테스트 — 정상 응답 / 데이터 없음·404 / 유효성 실패 / 네트워크 오류 케이스 검증

- **Task 006: 인증 및 권한 시스템 구현**
  - 사용자 인증 시스템 구축
  - 권한 기반 접근 제어 구현
  - 보안 미들웨어 및 세션 관리
  - **(구현 직후 필수 테스트)** Playwright MCP로 인증 플로우 E2E 테스트 — 로그인 성공 / 실패 / 비인가 접근 차단 / 세션 만료 케이스 검증

- **Task 006-1: 핵심 기능 통합 테스트**
  - Playwright MCP를 사용한 전체 사용자 플로우 E2E 테스트 (시나리오별 단계 명시)
  - API 연동 및 비즈니스 로직 검증 (정상 경로 + 예외 처리)
  - 에러 핸들링 및 엣지 케이스(경계값, 동시성, 빈 데이터) 테스트
  - 모든 시나리오 통과 확인 후 Phase 완료 처리
...
5. **테스트 동반**: 구현 Task는 반드시 Playwright MCP 테스트 항목과 **"## 테스트 체크리스트"** 섹션을 포함 (테스트 통과가 완료 기준)
...
- [ ] 모든 구현 Task에 "구현 직후 Playwright MCP 테스트 수행" 단계가 명시되었는가?
- [ ] API 연동 및 비즈니스 로직 Task가 정상/에러/엣지 케이스를 모두 포함해 꼼꼼히 검증하도록 작성되었는가?
- [ ] 모든 기능 테스트가 Playwright MCP를 사용하도록 명시되었는가?
- [ ] 각 작업 파일에 "## 테스트 체크리스트" 섹션이 명시되었는가?
- [ ] 모든 사용자 플로우에 대한 E2E 테스트 시나리오가 정의되었는가?
- [ ] 에러 핸들링 및 엣지 케이스 테스트가 고려되었는가?
- [ ] "테스트 통과 전 완료(✅) 표시 금지" 원칙이 반영되었는가?
